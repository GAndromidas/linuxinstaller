...
alias mirror='country=$(curl -s --max-time 5 https://ipinfo.io/country 2>/dev/null | tr -d '"\n\r") && [ -n "$country" ] && [ "$country" != "null" ] && sudo reflector --latest 10 --sort rate --age 24 --country "$country" --save /etc/pacman.d/mirrorlist --protocol https,http || sudo reflector --latest 10 --sort rate --age 24 --save /etc/pacman.d/mirrorlist --protocol https,http && sudo pacman -Syy'
